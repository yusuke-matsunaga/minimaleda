#ifndef LIBYM_BDD_BMM_BDDMGRMODERN_H
#define LIBYM_BDD_BMM_BDDMGRMODERN_H

/// @file libym_bdd/bmm/BddMgrModern.h
...
inline
void
BddMgrModern::activate(tBddEdge vd)
{
  Node* vp = get_node(vd);
  if ( vp && vp->linkinc() == 1 ) {
    lockall(vp);
...
inline
void
BddMgrModern::deactivate(tBddEdge vd)
{
  Node* vp = get_node(vd);
  if ( vp && vp->linkdec() == 0 ) {
    unlockall(vp);
...
inline
BmmNode*
BddMgrModern::get_node(tBddEdge e)
{
  return (Node*)(e & ~3);
}
...
inline
void
BddMgrModern::setmark(tBddEdge vd)
{
  Node* vp = get_node(vd);
...
#endif // LIBYM_BDD_BMC_BDDMGRMODERN_H

...
tVarSize
BddMgrClassic::mark_support(tBddEdge e)
{
  clear_varmark();
  sup_step(e);
  clear_pnmark(e);
...
tVarSize
BddMgrClassic::mark_support(const list<tBddEdge>& edge_list)
{
  clear_varmark();
  // サポート変数にマークをつける．
...
// サポート変数に印をつける．
void
BddMgrClassic::sup_step(tBddEdge e)
{
  for ( ; ; ) {
    Node* vp = get_node(e);
    if ( vp == 0 || vp->pmark() ) {
...
void
BddMgrClassic::scc_step(tBddEdge e,
			tBddEdge s)
{
  if ( check_one(e) ) {
    while ( !check_one(s) ) {
...
// メモリ不足のためにエラーとなる可能性がある．
tBddEdge
BddMgrClassic::esmooth(tBddEdge e1,
		       tBddEdge e2)
{
...
tBddEdge
BddMgrClassic::esmooth_step(tBddEdge e)
{
  if ( check_leaf(e) ) {
    return e;
  }
...
tBddEdge
BddMgrClassic::and_exist(tBddEdge e1,
			 tBddEdge e2,
			 tBddEdge e3)
{
...
tBddEdge
BddMgrClassic::andexist_step(tBddEdge f,
			     tBddEdge g)
{

...
/// @file libym_bdd/bmc/bmc_isop.cc
...
LogExpr
BddMgrClassic::prime_cover(tBddEdge l,
			   tBddEdge u)
{
...
tBddEdge
BddMgrClassic::minimal_support(tBddEdge l,
			       tBddEdge u)
{
  if ( check_error(l) || check_error(u) ) {
    return kEdgeError;
...
// isop 用の再帰関数
tBddEdge
BddMgrClassic::isop_step(tBddEdge l,
			 tBddEdge u,
			 LogExpr& cov)
{
  if ( check_zero(l) ) {
...
// prime_cover 用の再帰関数
tBddEdge
BddMgrClassic::pc_step(tBddEdge l,
		       tBddEdge u,
		       LogExpr& cov)
{
  if ( check_zero(l) ) {

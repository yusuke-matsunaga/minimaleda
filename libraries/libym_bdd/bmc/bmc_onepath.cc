...
/// @file libym_bdd/bmc/bmc_onepath.cc
...
tBddEdge
BddMgrClassic::onepath(tBddEdge e)
{
  if ( check_error(e) ) {
    return kEdgeError;
  }
...
// 結果はその経路を表す論理積の形のBDDで返される．
tBddEdge
BddMgrClassic::shortest_onepath(tBddEdge e)
{
  if ( check_error(e) ) {
    return kEdgeError;
  }
...
tBddEdge
BddMgrClassic::sp_step(tBddEdge e,
		       hash_map<tBddEdge, tBddEdge>& sp_assoc)
{
  if ( check_one(e) ) {
    return kEdge1;
...
// sp_step 中で用いられる関数
ymint
BddMgrClassic::sp_len(tBddEdge e)
{
  if ( check_zero(e) ) {
    return -1;
  }
...
// 最短1パスの長さを求める．
tVarSize
BddMgrClassic::shortest_onepath_len(tBddEdge e)
{
  if ( check_invalid(e) ) {
    return 0;
  }
...
ymint
BddMgrClassic::spl_step(tBddEdge e,
			hash_map<tBddEdge, ymint>& assoc)
{
  if ( check_one(e) ) {
    return 0;

...
///
/// $Id: genpat.cc 2507 2009-10-17 16:24:02Z matsunaga $
///
...
// node の部分グラフで fv を実現しているものがあったら true を返す．
bool
...
// node1 と node2 の部分グラフを組み合わせて fv が実現できたら
// true を返す．
bool
...
// node1 の部分グラフと node2 の部分グラフを組み合わせて fv が実現できたら
// true を返す．
bool
...
struct PatList
{
  ...
  {
    ymuint32 size = h.node()->size();
    if ( mMinSize > size ) {
      mMinSize = size;
    }
    ymuint32 level = h.node()->level();
    if ( mMinLevel > level ) {
      mMinLevel = level;
    }
    ymuint32 slack = 1;
    if ( size <= mMinSize + slack ) {
      mList.push_back(h);
      return true;
    }
    return false;
...
genpat(ymuint ni)
{
  ...
  for (ymuint rpos = 0; rpos < pat_list.size(); ++ rpos) {
    ...
    for (ymuint i =  0; i < rpos; ++ i) {
      GpNode* node2 = pat_list[i];
#if 0
      cout << "node1: " << rpos << ": " << hex << node1->pat() << dec << endl
	   << "node2: " << i << ": " << hex << node2->pat() << dec << endl;
#endif
      // node1 と node2 を子供とするパタンを生成する．
      // ただしファンインの極性の組み合わせが4通りある．
      for (ymint pp = 0; pp < 4; ++ pp) {
	ymuint32 fv1 = node1->pat();
	bool inv1 = false;
	if ( pp & 1U ) {
	  fv1 = ~fv1;
	  inv1 = true;
	}
	ymuint32 fv2 = node2->pat();
	bool inv2 = false;
	if ( pp & 2U ) {
	  fv2 = ~fv2;
	  inv2 = true;
	...
	// node1 もしくはその TFI で fv を実現していたら冗長
	if ( check_fv(node1, fv0) ) {
	  continue;
	}
	// node2 もしくはその TFI で fv を実現していたら冗長
	if ( check_fv(node2, fv0) ) {
	  continue;
	}
	// node1 と node2 の TFI を組み合わせると fv が実現できたら冗長
	if ( check_fv2(node1, node2, fv0, mask, true) ) {
	  continue;
	}
	GpHandle h = gpmgr.make_and(GpHandle(node1, inv1),
				    GpHandle(node2, inv2),
				    fv);
	if ( inv ) {
	  h = ~h;
	}
	if ( func_table[fv0].push_back(h) ) {
	  pat_list.push_back(h.node());
#if 0
	  cout << "new pat: " << h.node()->size() << ": "
	       << hex << fv << dec
	       << ": " << rpos << ", " << i
	       << endl;
#endif
  ...
  for (ymuint i = 2; i < nf; i += 2) {
    ...
    total_level += func_table[i].mMinLevel;
  }
  cout << "total cost = " << total_cost << endl;
  cout << "total cost2 = " << total_level << endl;
}

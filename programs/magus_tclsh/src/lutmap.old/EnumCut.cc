...
void
EnumCut::mark_cnode(SbjNode* node)
{
...
void
EnumCut::mark_cnode2(SbjNode* node)
{
  if ( !temp1mark(node) ) {
    set_temp1mark(node);
    mMarkedNodes[mMarkedNodesLast] = node;
...
void
EnumCut::mark_cnode3(SbjNode* node)
{
  const vector<SbjNode*>& cnode_list1 = cnode_list(node);
  for (vector<SbjNode*>::const_iterator p = cnode_list1.begin();
       p != cnode_list1.end(); ++ p) {
...
void
EnumCut::get_frontier(SbjNode* node,
		      ymuint root_depth)
{
  if ( state(node) ) {
    return;
...
void
EnumCut::clear_frontier(SbjNode* node)
{
  if ( state(node) ) {
    clear_state(node);
    if ( node->is_logic() ) {
...
inline
void
EnumCut::push_node(SbjNode* node)
{
...
void
EnumCut::set_cut_node_list_recur(SbjNode* node,
